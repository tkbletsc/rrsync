...
This will mean that any use of the corresponding SSH key can ONLY run that command. Further, the rrsync.py script itself ONLY allows itself to be run when `ssh` is being called by `rsync`. It examines what `rsync` is trying to do, and will only execute the real `rsync` server if it's allowed. The restrictions are to (1) ensure that the thing being copied is under the given `topdir` path and (2) that, if `-ro` is given, only reads are permitted. 
